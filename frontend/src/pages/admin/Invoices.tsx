...
const Invoices: React.FC = () => {
  ...
  const [newInvoice, setNewInvoice] = useState({ title: "", contract_id: "", currency: "INR" })
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowCreate(false)
      setNewInvoice({ title: "", contract_id: "", currency: "INR" })
            ...
            <tbody className="divide-y divide-gray-200">
              {invoices.map((inv) => (
                <tr key={inv.id} className="bg-white">
                  ...
                  <td className="px-4 py-3 text-right text-gray-900 font-medium">{fmtMoney(inv.total)}</td>
                  <td className="px-4 py-3 text-right text-gray-700">{fmtMoney(inv.balance)}</td>
